...
{
    //j'ai toujours dit que programmer �a forgeait le charact�re
...
SDL_Surface* font::render_text(string text)
{
    ...
    final_img = SDL_CreateRGBSurface(SDL_SWSURFACE, final_x, y, game->screen->format->BitsPerPixel,
                                     game->screen->format->Rmask, game->screen->format->Gmask,
                                     game->screen->format->Bmask, game->screen->format->Amask);
    SDL_Rect src, dest;
    src.x = 0;
    src.y = 0;
    src.h = y;
