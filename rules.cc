...
bool rules::set_move(Uint8 oldx, Uint8 oldy, Uint8 x, Uint8 y)
{
	...
	{
#ifdef DEBUG
		cout << "move from (" << (Uint32)oldx << "," << (Uint32)oldy << ") to (" << (Uint32)x << "," << (Uint32)y <<
			") accepted" << endl;
#endif
		if (gametype == NETGAME)
		{
...
{
#ifdef DEBUG
	cout << "player: " << (CURRENT_PLAYER) << " moving from: " << (Uint32)ox << "," << (Uint32)oy << " to " << (Uint32)
		nx << "," << (Uint32)ny << endl;
#endif
	if (!valid_move())
	{
...
#ifdef DEBUG
	printf("Now fork: %s %s %s %s\n", strategy_bin, blobs.serialize().c_str(), holes.serialize().c_str(),
	       cplayer.c_str());
#endif
	int childPid = fork();
	if (childPid == 0) // Child process
	{
		execl(strategy_bin, strategy_bin, blobs.serialize().c_str(), holes.serialize().c_str(), cplayer.c_str(),
		      (char*)NULL);
	}
...
#ifdef DEBUG
	cout << "computer computed move from: " << (Uint32)ox << "," << (Uint32)oy << " to " << (Uint32)nx << "," << (
		Uint32)ny << endl;
#endif
