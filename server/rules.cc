...
bool rules::set_move(Uint8 oldx, Uint8 oldy, Uint8 x, Uint8 y)
{
    ...
    if (blobs[oldx][oldy] != get_current_player())
    {
        ...
        cout << "blob at " << (Uint32)oldx << " " << (Uint32)oldy << " is " << (Uint32)blobs[oldx][oldy] <<
            " and you are " << get_current_player() << endl;
#endif
        return false;
    }
    ...
    if (d <= 4)
    {
        //move accepted
#ifdef DEBUG
        cerr << "move from (" << (Uint32)oldx << "," << (Uint32)oldy << ") to (" << (Uint32)x << "," << (Uint32)y <<
            ") accepted" << endl;
#endif
        ox = oldx;
        oy = oldy;
        ...
        turntime += period;
        cout << "at " << turntime << " :";
        cout << "turn: " << turn_number << " player: " << (turn_number % number_of_players) << " moved from (" << (
            Uint32)ox << "," << (Uint32)oy << ") to (" << (Uint32)nx << "," << (Uint32)ny << ")" << endl;
        return true;
    }
...
{
#ifdef DEBUG
    cerr << "player: " << (turn_number % number_of_players) << "moving from: " << (Uint32)ox << "," << (Uint32)oy <<
        "to " << (Uint32)nx << "," << (Uint32)ny << endl;
#endif
